               ...
               <div className="mt-6 space-y-4 rounded-lg bg-muted p-4 text-center">
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                   <div>
                     ...
                     <p className="text-2xl font-bold text-primary">{interestResult.interest.toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</p>
                   </div>
                   <div>
                     <p className="text-sm text-muted-foreground">{t('totalValue')}</p>
                     <p className="text-2xl font-bold text-primary">{interestResult.total.toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</p>
                   </div>
                 </div>
               </div>
            )}
             ...
             {loanResult && (
               <div className="mt-6 space-y-4 rounded-lg bg-muted p-4">
                 <div className="text-center mb-4">
                    ...
                    <p className="text-3xl font-bold text-primary">{loanResult.monthlyPayment.toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</p>
                 </div>
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-center">
                   <div>
                     <p className="text-sm text-muted-foreground">{t('totalPayment')}</p>
                     <p className="text-lg font-semibold">{loanResult.totalPayment.toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</p>
                   </div>
                   <div>
                     <p className="text-sm text-muted-foreground">{t('totalInterest')}</p>
                     <p className="text-lg font-semibold">{loanResult.totalInterest.toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</p>
                   </div>
                 </div>
               </div>
            )}
             ...
             {tipResult && (
               <div className="mt-6 space-y-4 rounded-lg bg-muted p-4">
                 <div className="text-center mb-4">
                    ...
                    <p className="text-3xl font-bold text-primary">{tipResult.perPerson.toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</p>
                 </div>
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-center">
                   <div>
                     <p className="text-sm text-muted-foreground">{t('tipAmount')}</p>
                     <p className="text-lg font-semibold">{tipResult.tipAmount.toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</p>
                   </div>
                   <div>
                     <p className="text-sm text-muted-foreground">{t('totalBill')}</p>
                     <p className="text-lg font-semibold">{tipResult.total.toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</p>
                   </div>
                 </div>
               </div>
            )}
